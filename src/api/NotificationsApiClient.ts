...
export class NotificationsApiClient {
  ...
  async getLastNotification(): Promise<EventResponse> {
    ...
    return await this.httpClient.getRequest<EventResponse>(path)
  }

  async getPaginatedNotifications(querySize: number, querySince?: string): Promise<NotificationsResponse> {
    const storedDeviceId = this.appProperties.getDeviceId()
