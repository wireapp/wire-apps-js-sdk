...
export class AssetsTransferService {
  ...
  async downloadAsset(assetRemoteData: AssetRemoteData): Promise<Uint8Array> {
    ...
    if (!HashUtils.isHashEqual(assetRemoteData.sha256, calculatedSha256)) {
      throw new InvalidParameterError(
        `Asset download failed. Asset checksums do not match. assetId: ${obfuscateId(assetRemoteData.assetId)}`
      )
    }
  ...
  async uploadAssetForSending(asset: AssetData): Promise<AssetRemoteData> {
  ...
  private async uploadAsset(asset: AssetData, assetUploadData: AssetUploadData): Promise<AssetUploadResponse> {
