...
export class CoreCryptoService {
  ...
  async initOrRegisterClient() {
    ...
    if (this.appProperties.hasDeviceId()) {
      ...
      const cryptoClientId = CryptoClientId.create(appQualifiedId.id, storedDeviceId, appQualifiedId.domain)
      ...
      this.logger.info(`Initializing Proteus Client. appId: ${obfuscateId(appQualifiedId.id)}`)
      ...
      const cryptoClientId = CryptoClientId.create(appQualifiedId.id, registeredDeviceId, appQualifiedId.domain)

      this.logger.info(
        `Initializing MLS Client. userId: ${obfuscateId(appQualifiedId.id)}, deviceId: ${obfuscateClientId(registeredDeviceId)}`
      )
  ...
  async processWelcomeMessage(welcomeMessageBytes: Uint8Array, groupInfoBytes: Uint8Array) {
    try {
  ...
  async conversationExists(mlsGroupId: string) {
    const mlsGroupIdBytes = Decoder.fromBase64(mlsGroupId).asBytes
    return await this.coreCryptoClient!.conversationExists(new ConversationId(mlsGroupIdBytes))
  }

  async conversationEpoch(mlsGroupId: string) {
    const mlsGroupIdBytes = Decoder.fromBase64(mlsGroupId).asBytes
    return await this.coreCryptoClient!.conversationEpoch(new ConversationId(mlsGroupIdBytes))
  }

  async joinMlsConversation(groupInfoBytes: Uint8Array): Promise<void> {
    await this.coreCryptoClient!.joinMlsConversation(new GroupInfo(groupInfoBytes))
  }
  ...
  async addClientsToMlsConversation(
    ...
    const claimedKeyPackagesResult = await this.mlsService.claimKeyPackages(members, this.defaultCiphersuiteCode!)
    ...
    this.logger.debug(
      `Added ${claimedKeyPackagesResult.successUsers.length} clients to MLS group id: ${obfuscateId(mlsGroupId)}`
    )
    return {
  ...
  async establishMlsConversation(
    ...
    if (removalKey == null) {
      throw new MissingParameterError(
        `No Public Keys found, skipping creating a conversation. mlsGroupId: ${obfuscateId(mlsGroupId)}`
      )
    }
    ...
    try {
      ...
      if (
        CoreCryptoError.Mls.instanceOf(exception) &&
        MlsError.ConversationAlreadyExists.instanceOf(exception.inner.mlsError)
      ) {
